...
export async function updateTimelineItemIcon(formData: FormData) {
  const file = formData.get("file") as File | null;
  const id = stringFromForm(formData, "id");
  if (!file || !id) return;

  const result = await uploadMedia(file, "home/timeline/icons");
  if ("error" in result) return;

  const supabase = await createClient();
  await supabase.from("career_timeline").update({ icon_media_id: result.id }).eq("id", id);
  revalidatePath(PATH);
}

export async function removeTimelineItemIcon(formData: FormData) {
  const id = stringFromForm(formData, "id");
  if (!id) return;
  const supabase = await createClient();
  await supabase.from("career_timeline").update({ icon_media_id: null }).eq("id", id);
  revalidatePath(PATH);
}
