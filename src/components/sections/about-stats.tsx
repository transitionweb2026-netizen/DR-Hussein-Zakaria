...
import { getPublishedHomeStats } from "@/lib/data/home";
import { pickLocale } from "@/lib/i18n-content";
import { getIcon } from "@/lib/icon-map";

/** Same shared statistic numbers as the Home page's Statistics section
 * (objective facts about the doctor, not page-specific content), with this
 * page's own heading (about_stats_section). */
export async function AboutStats() {
  const locale = await getLocale();
  const [section, stats] = await Promise.all([getAboutStatsSection(), getPublishedHomeStats()]);
  ...
  const slides = stats.map((item) => {
    ...
    return (
      <GlassCard key={item.id} hover className="flex w-[220px] flex-col items-center gap-4 p-7 text-center sm:w-[240px]">
        <IconTile icon={Icon} />
        <Counter value={item.value} suffix={item.suffix} className="text-3xl font-extrabold text-ink-900 sm:text-4xl" />
